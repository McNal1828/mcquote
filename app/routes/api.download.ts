...
export async function action({ request }: Route.ActionArgs) {
    ...
    if (type === "cost") {
        ...
        const pristineModel = JSON.parse(JSON.stringify(templateSheet.model));

        const groupedProducts: Record<string, any[]> = Array.isArray(products)
            ? { "원가표": products }
            : products;

        // 1. 모든 워크시트를 먼저 생성 및 복사합니다 (ID 충돌 방지 포함)
        const worksheets: Record<string, ExcelJS.Worksheet> = {};
        let isFirst = true;
        for (const groupName of Object.keys(groupedProducts)) {
            if (isFirst) {
                templateSheet.name = groupName;
                worksheets[groupName] = templateSheet;
                isFirst = false;
            } else {
                const ws = workbook.addWorksheet(groupName);
                const originalId = ws.id;
                const modelToAssign = JSON.parse(JSON.stringify(pristineModel));
                modelToAssign.id = originalId;
                modelToAssign.name = groupName;
                ws.model = modelToAssign;
                ws.name = groupName;

                // 병합 셀 명시적 복사 (ExcelJS 모델 복사 버그 우회)
                if (pristineModel.merges) {
                    pristineModel.merges.forEach((merge: string) => {
                        try {
                            ws.mergeCells(merge);
                        } catch (e) {}
                    });
                ...
                worksheets[groupName] = ws;
            }
        }

        // 2. 각 워크시트에 데이터를 기입합니다.
        for (const [groupName, prods] of Object.entries(groupedProducts)) {
            const worksheet = worksheets[groupName];

            // 1. 3행을 템플릿으로 사용하여, 4행부터 제품 개수만큼 행을 삽입합니다.
            if (prods && prods.length > 1) {
                const emptyRows = Array(prods.length - 1).fill([]); // 템플릿 행(1개)을 제외한 나머지 개수만큼 빈 행 추가
            ...
            const totalRowIndex = 3 + prods.length;
        ...
        for (const [groupName, prods] of Object.entries(groupedProducts)) {
            ...
            (worksheet.headerFooter as any).scaleWithDoc = true; // 바닥글 문서에 맞게 배율 조정(L)
            ...
            prods.forEach((prod: any, idx: number) => {
                ...
                const dcWon = Number(prod.DC원화) || 0;
                row.getCell(11).value = dcWon / 100; // K: DC원화 (%)
                row.getCell(9).value = {
            ...
            if (prods && prods.length > 0) {
                const shiftCount = prods.length - 1;
                const subtotalRowIndex = 37 + shiftCount;
                const vatRowIndex = 38 + shiftCount;
                const totalRowIndex = 39 + shiftCount;
            ...
            const shrinkRatio = 64 / 73;

            if (tempWorksheet.properties?.defaultColWidth) {
                if (!worksheet.properties) worksheet.properties = {} as any;

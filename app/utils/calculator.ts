...
/**
 * 원화PPC 또는 마진율이 변경되었을 때 DC원화를 역산하여 돌려주는 공통 함수
 */
...
/**
 * DB에서 로드된 다양한 형식의 제품 정보(Legacy Array, Grouped Record 등)를
 * 일관되게 Record<string, any[]> 형식으로 가공하여 반환하는 헬퍼 함수
 */
export function normalizeProducts(products: any): Record<string, any[]> {
    if (Array.isArray(products)) {
        return { "원가표1": products };
    }
